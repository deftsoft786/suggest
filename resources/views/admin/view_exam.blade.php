  ...
  @include('admin/header')
                    ...
                    <div class="main-bar">
                        ...
                        <div class="col-lg-6 col-md-8 col-sm-8">
                            <ol class="breadcrumb float-right nav_breadcrumb_top_align">
                                ...
                                <li class="breadcrumb-item">
                                    <a href="{{url('admin/view_exam')}}"> View Exam</a>
                                </li>
                                ...
                                <div class="card m-t-35">
                                    <div class="card-header bg-white">
                                       <span class='pull-left'>
                                            <i class="fa fa-book"></i> View Exam
                                        </span>
                                        <span class='pull-right'>
                                             <a class="btn btn-primary" href="{{url('admin/add_exam')}}"> Add </a>
                                        </span>
                      ...
                      <div class="card-block p-t-10">
                                        <div class=" m-t-25">
                                            <table class="table table-striped table-bordered table-hover " id="sample_6">
                                                <thead>
                      <form action="{{url('admin/view_exam')}}" method="post" class="form-horizontal cat" id="form_block_validator">
                                 ...
                                 <option value="all"> All </option>
                                   ...
                                   <tbody>
                                     @if(count($data) > 0)
                                       ...
                                       <tr>
     <td>@if(!empty($value->exam)) {{$value->exam}} @else  N/A @endif</td>
     
     <td>@if(!empty($value->category_name->category_name)){{$value->category_name->category_name}} @else N/A @endif</td>
     <td>@if(!empty($value->description)) <?php echo str_limit($value->description, 150); ?> @else  N/A @endif</td>
     <td>@if(!empty($value->result_date)) {{$value->result_date}} @else  N/A @endif</td>
     <td class="action-icons text-center">
        <span>
             <a href="{{url('admin/edit_exam/'.$value->id)}}">
        </span>
        <i style="color:blue;" class="fa fa-pencil-square-o fa-lg" title="Edit Detail" aria-hidden="true"></i> </a>
        <a onclick="return confirm('Are you sure you want to Delete Exam?')" style="text-decoration:none;" href="{{ url('admin/delete_exam/'.$value->id)}}">  </span><i style="color:red;" title="Delete" class="fa fa-trash-o fa-lg" aria-hidden="true"></i> </a>
                                        </td>
                                       </tr>
                                     @endforeach
                                     @endif
    ...
    <!-- global scripts-->
      ...
      $(document).ready(function(){
        $('#cat').change(function() {
        var i =  $('#cat').val();
        $(".cat").attr("action", "{{url('admin/view_exam')}}?Category="+i) ;
        $('.cat').submit();
