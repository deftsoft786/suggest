                            ...
                            <div class="col-12 data_tables">
                                ...
                                <div class="card m-t-35">
                                    <div class="card-header bg-white">
                                        <i class="fa fa-comments"></i> Review Management Table
...
@foreach($data as $value)
                                    <tr>
                                         ...
                                         <td>@if(!empty($value->review)) {{$value->review}} @else  N/A @endif</td>
                                         <td>@if(!empty($value->ranking)) {{$value->ranking}} @else  N/A @endif</td>
                                         <td>@if(!empty($value->status == 1)) {{'Approved'}} @elseif($value->status == 2)  {{'Pending'}} @else Disapproved @endif</td>
                                        
                                          <td class="action-icons text-center"><span> 

                                            <a onclick="return confirm('Are you sure you want to Approve Review?')" style="text-decoration:none;" href="{{ url('admin/approve_review/'.$value->id) }}">  </span><i style="color:green;" title="Approve" class="fa fa-check fa-lg" aria-hidden="true"></i>
 </a> &nbsp
                                             <a onclick="return confirm('Are you sure you want to Disapprove Review?')" style="text-decoration:none;" href="{{ url('admin/decline_review/'.$value->id) }}">  </span><i style="color:orange;" title="Disapprove" class="fa fa-window-close fa-lg" aria-hidden="true"></i>
                                                </a> &nbsp

                                             <a onclick="return confirm('Are you sure you want to Delete Review?')" style="text-decoration:none;" href="{{ url('admin/delete_review/'.$value->id) }}">  </span><i  style="color:red;" title="Delete" class="fa fa-trash-o fa-lg" aria-hidden="true"></i>
 </a>
